...
def set_mask(edge_index, mask, model):
...
class GINEncoder(BasicEncoder):
    ...
    def __init__(self, config: Union[CommonArgs, Munch], *args):
        ...
        self.linear_encoder = nn.Linear(config.dataset.dim_node, config.model.dim_hidden)


        self.convs = nn.ModuleList(
            [
                GINConv(nn.Sequential(nn.Linear(config.dataset.dim_node, 2 * config.model.dim_hidden),
        ...
        # --- SubGNN choosing masks, part 1 ---
        self.num_component = num_component = 4
        self.linear_project = nn.Linear(config.model.dim_hidden, config.model.dim_hidden)
        self.attn_mlp = nn.Sequential(nn.Linear(2 * config.model.dim_hidden, 4 * config.model.dim_hidden),
                                      nn.BatchNorm1d(4 * config.model.dim_hidden), nn.ReLU(),
                                      nn.Linear(4 * config.model.dim_hidden, num_component))
        self.edge_attn_logits = None
        self.edge_attn_masks = None
    ...
    def forward(self, x, edge_index, batch):
        ...
        if at_stage(1, self.config):
            out_readout = self.readout(layer_feat[-1], batch).repeat(1, self.num_component)
        else:
            # --- SubGraph ---
            layer_feat[-1].detach()
            # --- Block gradients to subnetwork masks before ---
            row, col = edge_index
            hi, hj = layer_feat[-1][row], layer_feat[-1][col]
            hil, hjl = self.linear_project(hi), self.linear_project(hj)
            self.edge_attn_logits = self.attn_mlp(torch.cat([hil, hjl], dim=1))
            if at_stage(2, self.config):
                if self.training or not self.training:
                    self.edge_attn_masks = gumbel_sigmoid(self.edge_attn_logits.T)
                else:
                    self.edge_attn_masks = self.edge_attn_logits.T > 0
            elif at_stage(3, self.config):
                self.edge_attn_masks = self.edge_attn_logits.T > 0

            g_reprs = []
            for i in range(self.num_component):
                new_edge_index = set_mask(edge_index, self.edge_attn_masks[i], self)
                layer_feat = [x]
                for i, (conv, batch_norm, relu, dropout) in enumerate(
                        zip(self.convs, self.batch_norms, self.relus, self.dropouts)):
                    post_conv = batch_norm(conv(layer_feat[-1], new_edge_index))
                    if i != len(self.convs) - 1:
                        post_conv = relu(post_conv)
                ...
                g_reprs.append(self.readout(layer_feat[-1], batch))
                clear_mask(self)

            out_readout = torch.stack(g_reprs, dim=0)
