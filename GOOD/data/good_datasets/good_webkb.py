...
class DomainGetter(object):
    ...
    def get_university(self, graph: Data) -> int:
        """
        Args:
...
class GOODWebKB(InMemoryDataset):
    r"""
    The GOOD-WebKB dataset adapted from the `Geom-GCN: Geometric Graph Convolutional Networks
    <https://openreview.net/forum?id=S1e2agrFvS>`_.
    ...
    def get_covariate_shift_graph(self, sorted_data_list, graph):
        ...
        for i, data in enumerate(sorted_data_list):
            train_val_test_list[data.domain_id].append(data)
        ...
        train_list = train_list + ood_val_list
    ...
    def process(self):
        ...
        graph = Data(x=pseudo_batch.x, edge_index=pseudo_batch.edge_index, y=pseudo_batch.y, university=university)
    ...
    def load(dataset_root: str, domain: str, shift: str = 'no_shift', generate: bool = False):
        ...
        dataset = GOODWebKB(root=dataset_root, domain=domain, shift=shift, generate=generate)
