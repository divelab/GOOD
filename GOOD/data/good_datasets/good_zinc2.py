...
class GOODZINC2(InMemoryDataset):
    r"""
    The GOOD-ZINC dataset adapted from `ZINC database
    <https://pubs.acs.org/doi/abs/10.1021/acs.jcim.5b00559>`_.
    ...
    def get_concept_shift_list(self, sorted_domain_split_data_list):
        ...
        sorted_pyx = np.sort(global_pyx)
        global_mid_pyx = sorted_pyx[len(global_pyx) // 2]

        # sorted_domain_split_data_list = sorted(sorted_domain_split_data_list, key=lambda domain_data: domain_data[-1], reverse=)

        bias_connect = [0.95, 0.9, 0.95, 0.85, 0.5]
        is_train_split = [True, True, False, True, False]
            ...
            for domain_id, each_domain_datas in enumerate(sorted_domain_split_data_list):
                ...
                for i in range(len(each_domain_datas) - 1):
                    ...
                    if cur_split == len(env_list) - 1:
                        ...
                        oc = order_connect[cur_split][domain_id]
                        if oc * (data.pyx - global_mid_pyx) > 0:
                        ...
                        elif oc * (data.pyx - global_mid_pyx) < 0:
    ...
    def get_domain_sorted_list(self, data_list, domain='scaffold'):
        ...
        sorted_data_list, sorted_domain_split_data_list = self.filter_domains(sorted_domain_split_data_list, domain)

        return sorted_data_list, sorted_domain_split_data_list

    def filter_domains(self, sorted_domain_split_data_list, domain):
        filtered_sorted_ds_data_list = []
        filtered_sorted_data_list = []
        domain_id = 0
        for domain_data in sorted_domain_split_data_list:
            if 10000 <= len(domain_data):
                filtered_sorted_ds_data_list.append(domain_data)
                for datum in filtered_sorted_ds_data_list[domain_id]:
                    datum.domain_id = torch.LongTensor([domain_id])
                    filtered_sorted_data_list.append(datum)
                domain_id += 1
        filtered_sorted_data_list = sorted(filtered_sorted_data_list, key=lambda datum: getattr(datum, domain))
        return filtered_sorted_data_list, filtered_sorted_ds_data_list

    def process(self):
        dataset = ZINC(root=self.root, name=self.mol_name)
